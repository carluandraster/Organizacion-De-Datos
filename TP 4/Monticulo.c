 ...
 *  - Si el dato proporcionado es menor al dato del nodo padre, no se realiza ninguna acción
 */
void setHijoIzq(ArbolBinario padre, int dato)
{
    if (padre != NULL && dato >= padre->dato)
        if (padre->izq == NULL)
...
{
    if (nodo != NULL && (nodo->izq == NULL || dato <= nodo->izq->dato) && (nodo->der == NULL || dato <= nodo->der->dato))
    {
        nodo->dato = dato;
    }
}
